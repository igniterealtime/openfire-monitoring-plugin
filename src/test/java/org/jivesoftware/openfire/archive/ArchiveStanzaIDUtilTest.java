...
public class ArchiveStanzaIDUtilTest {
     ...
     * user does not contain an identifier: no identifier is added to such a message before it is routed, so none is
     * available to add to the copies of it that are delivered to its (local) sender.
     */
    @Test
    public void testAdjustDoesNotAddStanzaIdInSentCarbonOfMessageToRemoteEntity() {
        // Setup test fixture.
        final Message routed = newMessage(ROMEO, BASTANIO);
        final JID otherResource = new JID(ROMEO.toBareJID() + "/other");
        final Message carbon = newSentCarbon(routed, otherResource);
    ...
    /**
     * Asserts that two distinct messages that are addressed to an entity that is not a local user each obtain a
     * distinct identifier in the archive of their (local) sender.
     */
    @Test
    public void testUsesDistinctIdsForDistinctMessagesToRemoteEntity() throws Exception {
        // Setup test fixture.
    ...
    /**
     * Asserts that a (spoofed) stanza-id that claims to be generated by a local user is removed from a stanza that is
     * routed to a remote entity, even though no new identifier is generated for that (non-local) archive owner.
     */
    @Test
    public void testRemovesSpoofedStanzaIdsOfLocalUsersFromRoutedStanzaToRemoteRecipient() {
        // Setup test fixture.
        final Message input = newMessage(ROMEO, BASTANIO);
        addStanzaId(input, "spoofed-sender-id", ROMEO.toBareJID());
        addStanzaId(input, "remote-id", "remote.example");
    ...
    /**
     * Asserts that a spoofed stanza-id that survives routing to a remote entity is not later reused as the identifier
     * in the archive of the local sender: a fresh, random identifier is generated instead.
     */
    @Test
    public void testArchiveDoesNotReuseSpoofedStanzaIdAfterRoutingToRemoteEntity() throws Exception {
        // Setup test fixture.
        final Message input = newMessage(ROMEO, BASTANIO);
        addStanzaId(input, "spoofed-sender-id", ROMEO.toBareJID());

        // Execute system under test.
        ArchiveStanzaIDUtil.addStanzaIDToRoutedStanza(input, BASTANIO, IS_LOCAL_USER);
        final Message archived = parse(ArchiveStanzaIDUtil.getArchivableStanzaXml(input, IS_LOCAL_USER, ROMEO, BASTANIO));

        // Verify results.
        final String archivedId = StanzaIDUtil.findFirstUniqueAndStableStanzaID(archived, ROMEO.toBareJID());
        assertNotNull(archivedId);
        assertNotEquals("spoofed-sender-id", archivedId);
    }

    /**
     * Asserts that an identifier that claims to be generated by a local user is removed from a message that is
     * addressed to a chat room (a message that this plugin does not archive), while the identifier of the room itself
     * is retained.
     */
    @Test
    public void testRemovesSpoofedStanzaIdFromMessageToChatRoom() {
        // Setup test fixture.
        final Message input = newMessage(ROMEO, ROOM);
        input.setType(Message.Type.groupchat);
        addStanzaId(input, "spoofed-id", JULIET.toBareJID());
        addStanzaId(input, "room-provided-id", ROOM.toBareJID());

        // Execute system under test.
        final boolean result = ArchiveStanzaIDUtil.removeSpoofedStanzaIDs(input, IS_LOCAL_USER);
    ...
    /**
     * Asserts that an identifier that claims to be generated by a local user is removed from a message that has no
     * body (such as a chat state notification), which this plugin does not archive.
     */
    @Test
    public void testRemovesSpoofedStanzaIdFromMessageWithoutBody() {
        // Setup test fixture.
        final Message input = new Message();
        input.setFrom(ROMEO);
        input.setTo(JULIET);
        input.setType(Message.Type.chat);
        input.getElement().addElement("composing", "http://jabber.org/protocol/chatstates");
        addStanzaId(input, "spoofed-id", JULIET.toBareJID());
    ...
    /**
     * Asserts that an identifier that claims to be generated by a local user is removed from a message that was
     * received from a remote entity.
     */
    @Test
    public void testRemovesSpoofedStanzaIdFromMessageOfRemoteSender() {
        // Setup test fixture.
        final Message input = newMessage(BASTANIO, ROMEO);
        addStanzaId(input, "spoofed-id", ROMEO.toBareJID());
    ...
    /**
     * Asserts that identifiers that were generated by entities that are not local user accounts are retained.
     */
    @Test
    public void testRetainsStanzaIdsOfNonLocalUsers() {
        // Setup test fixture.
        final Message input = newMessage(BASTANIO, ROMEO);
        addStanzaId(input, "remote-id", "remote.example");
        addStanzaId(input, "room-provided-id", ROOM.toBareJID());
        addStanzaId(input, "domain-id", LOCAL_DOMAIN);
        ...
        // Verify results.
        assertFalse(result);
        assertEquals(3, getStanzaIdElements(input).size());
    }

    /**
     * Asserts that identifiers in a stanza that is forwarded by the message being processed are left as-is: those can
     * legitimately refer to a local user (as is the case for Message Carbons and MAM results).
     */
    @Test
    public void testRetainsStanzaIdsOfForwardedStanza() {
        // Setup test fixture.
        final Message routed = newMessage(ROMEO, JULIET);
        addStanzaId(routed, "juliets-id", JULIET.toBareJID());
        final JID otherResource = new JID(ROMEO.toBareJID() + "/other");
        ...
        // Execute system under test.
        final boolean result = ArchiveStanzaIDUtil.removeSpoofedStanzaIDs(carbon, IS_LOCAL_USER);

        // Verify results.
        assertFalse(result);
        assertEquals(1, getForwardedStanzaIdElements(carbon).size());
        assertEquals("juliets-id", StanzaIDUtil.findFirstUniqueAndStableStanzaID(getForwardedStanza(carbon), JULIET.toBareJID()));
    }
    ...
    public void testToleratesMalformedElements() {
        // Setup test fixture.
        final Message input = newMessage(ROMEO, JULIET);
        input.getElement().addElement(QName.get("stanza-id", "urn:xmpp:sid:0")).addAttribute("id", "no-by-attribute");
        addStanzaId(input, "unparsable-by", "not a jid@@@");
        addStanzaId(input, "spoofed-id", JULIET.toBareJID());
    ...
    /**
     * Asserts that a message that holds no identifiers at all is left untouched.
     */
    @Test
    public void testWithoutStanzaIds() {
        // Setup test fixture.
        final Message input = newMessage(ROMEO, JULIET);
        final String before = input.toXML();
    ...
    /**
     * Asserts that an identifier in a stanza that is forwarded by a stanza that was provided by another client is not
     * attributed to the entity that the stanza is delivered to: such an identifier was not generated by this server.
     */
    @Test
    public void testAdjustRemovesForgedStanzaIdFromClientSuppliedForward() {
        // Setup test fixture.
        final Message forged = newMessage(ROMEO, JULIET);
        addStanzaId(forged, "forged-id", JULIET.toBareJID());
        final JID attacker = new JID("mercutio@" + LOCAL_DOMAIN + "/street");
        final Message input = newCarbonEnvelope(forged, attacker, ROMEO);
        ...
        // Verify results.
        assertTrue(result);
        assertTrue(getForwardedStanzaIdElements(input).isEmpty());
        assertNull(StanzaIDUtil.findFirstUniqueAndStableStanzaID(getForwardedStanza(input), ROMEO.toBareJID()));
    }

    /**
     * Asserts that an identifier that is already attributed to the entity that a stanza is delivered to is removed
     * when it is part of a stanza that is forwarded by a stanza that was provided by another client.
     */
    @Test
    public void testAdjustRemovesForgedStanzaIdAttributedToRecipient() {
        // Setup test fixture.
        final Message forged = newMessage(ROMEO, JULIET);
        addStanzaId(forged, "forged-id", ROMEO.toBareJID());
        final JID attacker = new JID("mercutio@" + LOCAL_DOMAIN + "/street");
        final Message input = newCarbonEnvelope(forged, attacker, ROMEO);
    ...
    /**
     * Asserts that an identifier that was provided by a remote entity for a local user is removed from a stanza that
     * it forwards.
     */
    @Test
    public void testAdjustRemovesForgedStanzaIdFromRemotelyProvidedForward() {
        // Setup test fixture.
        final Message forged = newMessage(ROMEO, JULIET);
        addStanzaId(forged, "forged-id", ROMEO.toBareJID());
        final Message input = newCarbonEnvelope(forged, BASTANIO, ROMEO);
    ...
    /**
     * Asserts that a forged identifier is removed from a stanza that is forwarded by a stanza that is itself forwarded
     * by a stanza that was provided by a client.
     */
    @Test
    public void testAdjustRemovesForgedStanzaIdFromNestedForward() {
        // Setup test fixture.
        final Message forged = newMessage(ROMEO, JULIET);
        addStanzaId(forged, "forged-id", ROMEO.toBareJID());
        final JID attacker = new JID("mercutio@" + LOCAL_DOMAIN + "/street");
        final Message inner = newCarbonEnvelope(forged, ROMEO.asBareJID(), ROMEO);
        final Message input = newCarbonEnvelope(inner, attacker, ROMEO);
    ...
    /**
     * Asserts that the identifier of the archive owner is retained in the result of a query of that archive, which is
     * generated by this server rather than provided by a client.
     */
    @Test
    public void testAdjustRetainsStanzaIdInArchiveQueryResult() {
        // Setup test fixture.
        final Message archived = newMessage(JULIET, ROMEO);
        addStanzaId(archived, "romeos-id", ROMEO.toBareJID());
        final Message input = new Message();
        input.setFrom(ROMEO.asBareJID());
        input.setTo(ROMEO);
        final Element mamResult = input.getElement().addElement(QName.get("result", "urn:xmpp:mam:2"));
        mamResult.addAttribute("id", "romeos-id");
        final Element forwarded = mamResult.addElement(QName.get("forwarded", "urn:xmpp:forward:0"));
        forwarded.add(archived.getElement().createCopy());
        ...
        // Verify results.
        assertFalse(result);
        final Message forwardedStanza = new Message(mamResult
    ...
    /**
     * Creates a Message Carbons-like envelope that forwards the provided stanza. Unlike {@link #newSentCarbon(Message,
     * JID)}, the 'from' address can be chosen freely, which allows an envelope that was provided by a client (rather
     * than generated by the server) to be simulated.
     */
    private static Message newCarbonEnvelope(final Message original, final JID from, final JID to)
    {
