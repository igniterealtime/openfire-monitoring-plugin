...
/**
 * Integration tests for XEP-0313 MAM queries executed against a Members-Only MUC archive.
 *
...
public class MamForMembersOnlyRoomTest extends AbstractMamTest
{
    public MamForMembersOnlyRoomTest(SmackIntegrationTestEnvironment environment)
        throws Exception
    {
        super(environment);
    }
    ...
    {
        mucAddress = getRandomRoom("mam-filter-membersonly");
        mucAsSeenByOwner = mucManagerOne.getMultiUserChat(mucAddress);

        // Create members-only room
        createMembersOnlyMuc(mucAsSeenByOwner, nicknameOne);

        // Hand out memberships
    ...
    protected void postMessagePopulationRoomConfiguration() throws Exception
    {
        ...
        mucAsSeenByOwner.revokeMembership(conThree.getUser().asBareJid());
    }
     ...
     */
    @SmackIntegrationTest()
    public void testNonMemberCantQueryJidTextFilter() throws Exception
    {
        // Setup test fixture.
        final List<FormField> searchFields = List.of(FormField.textSingleBuilder("{urn:xmpp:fulltext:0}fulltext").setValue("apple").build());
        final MamManager.MamQueryArgs mamQueryArgs = MamManager.MamQueryArgs.builder()
