...
public class ArchiveStanzaIDUtil
{
     ...
     *
     * Only elements that are a direct child of the stanza are evaluated. Stanzas that are forwarded by the stanza that
     * is being processed (as used by, amongst others, Message Carbons and MAM results) can legitimately hold elements
     * that refer to a local user, and are left as-is.
     *
     * @param message The stanza that is being routed (cannot be null).
    ...
    private static boolean removeStanzaIDsOfLocalUsers( final Element parentElement, final Message message, final Predicate<JID> localUserTest )
    {
        ...
        while ( iterator.hasNext() ) {
            ...
            try {
                ...
                Log.debug( "Unable to parse the 'by' attribute value '{}' of a 'stanza-id' element as a JID. The element is left as-is.", by, e );
                continue;
            }
     ...
     * a stanza that holds no identifier at all.
     *
     * @param element The element to inspect (cannot be null).
