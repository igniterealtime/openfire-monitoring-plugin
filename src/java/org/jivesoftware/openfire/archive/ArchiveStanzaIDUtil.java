...
public class ArchiveStanzaIDUtil
{
    ...
    static boolean adjustForRecipient( final Message message, final JID recipient, final Predicate<JID> localUserTest )
    {
        ...
        try {
            ...
            return adjustRecursively( message.getElement(), recipient.toBareJID(), localUserTest, true );
        } catch ( Exception e ) {
     ...
     * @return true if at least one element was modified or removed, otherwise false.
     */
    private static boolean adjustRecursively( final Element stanzaElement, final String owner, final Predicate<JID> localUserTest, final boolean trusted )
    {
        boolean result;
        if ( trusted ) {
        ...
        for ( final Element forwardedStanza : findForwardedStanzas( stanzaElement ) ) {
            result |= adjustRecursively( forwardedStanza, owner, localUserTest, forwardedStanzasAreTrusted );
        }
        return result;
    }
