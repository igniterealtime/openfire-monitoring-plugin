...
public class JdbcPersistenceManager implements PersistenceManager {
     ...
     */
    public static void performDatabaseUpdateSplitJIDs()
    {
        ...
        try
        {
            ...
            for ( final String applicableColumnName : applicableColumnNames )
            {
                ...
                while ( rs.next() )
                {
                    ...
                    {
                        Log.debug("... processed {} rows so far.", progress);
                        lastProgressReport = Instant.now();
                    }
                }
                Log.debug( "Finished migrating all rows for the {} column. Processed {} messages in total.", applicableColumnName, progress );
                rs.close();
                pstmtUpdate.close();
                pstmtFind.close();
            }
